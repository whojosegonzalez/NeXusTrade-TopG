...
Status: **APPROVED BY USER ON 2026-09-24 — FROZEN PROTOCOL DESIGN**

Date: 2026-09-24  
Author: Planning and Implementation Developer  
Decision Owner: User / Human Reviewer (Approved and frozen)
...
| :------------------------------- | :---------- | :---------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------- | :----------------------------- | ---------------- | ----------------------------------------------------- |
| `market.priceUsd_anchor`         | Float (USD) | Instantaneous snapshot price: $P(t_0) > 0$.                                                                 | $t_{obs} \le t_0$; $                                                        | t*0 - t*{obs}                  | \le 60\text{s}$. | Baseline anchor price for forward return computation. |
...
### 9.1 Approval and Freeze Event

- **Status**: **APPROVED AND FROZEN ON 2026-09-24**.
- **Actor**: User / Human Reviewer.
- **Scope**: The frozen protocol design for Formulation B is accepted in full, fixing the non-parametric quantile selection predicate ($Q_3$), the 60-minute forward return evaluation horizon, and the 96-unit cohort planning parameters.
- **Authorized Next Step**: Preparation of a bounded **Scope Proposal for implementing and synthetically verifying the automated protocol validator and analysis tooling for Formulation B**.
...
### 9.3 Permitted Follow-On Work

Follow-on work is restricted to preparing the bounded tooling scope proposal under the H1–H4 engineering standards and isolated test framework (`scripts/verify-isolated.mjs`). Real network requests, production analyzer runs, and live data collection remain disabled.
